...
void AMAccelGyro::writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
...
uint8_t AMAccelGyro::readByte(uint8_t address, uint8_t subAddress) {
...
void AMAccelGyro::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest) {
	Wire.beginTransmission(address);   // Initialize the Tx buffer
	Wire.write(subAddress);            // Put slave register address in Tx buffer
	Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
	uint8_t i = 0;
...
void AMAccelGyro::readAccelData(int16_t * destination)
{
...
void AMAccelGyro::readGyroData(int16_t * destination)
{
...
int16_t AMAccelGyro::readTempData()
{
...
void AMAccelGyro::LowPowerAccelOnlyMPU6050()
{
...
void AMAccelGyro::calibrateMPU6050(float * dest1, float * dest2)
{
...
void AMAccelGyro::MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
	uint8_t rawData[4];
	uint8_t selfTest[6];
	float factoryTrim[6];
...
void AMAccelGyro::newMPUsetup(HardwareSerial& serial) {
	...
	serial.println(F("::: MPU-6050 6-DOF 16-bit :::"));
	serial.println(F("::: MPU-6050 motion sensor - 60 ug LSB :::"));
	uint8_t c = readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050);  // Read WHO_AM_I register for MPU-6050
	serial.print(F("MPU-6050 is : ")); serial.println(c, HEX);
	serial.print(F("MPU-6050 should be : ")); serial.println(0x68, HEX);
	delay(1000);
	if (c == 0x68) { // WHO_AM_I should always be 0x68
		...
		if (SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f && SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f) {
			serial.println(F("MPU-6050 Calibration Passed!"));
			delay(1000);
			calibrateMPU6050(gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers
			initMPU6050(); serial.println(F("MPU6050 initialized for active data mode....")); // Initialize device for active mode read of acclerometer, gyroscope, and temperature
		}
		else {
...
void AMAccelGyro::sensors3(HardwareSerial& serial) {
...
//		}
		...
		/*
		//SendSensors(serial);
		if (serial == Serial1) {      // --------- Set Node Command to "Send to Unity"
			serial.print("6,");
		}
